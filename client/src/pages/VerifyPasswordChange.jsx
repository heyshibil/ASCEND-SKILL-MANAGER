...
export default function VerifyPasswordChange() {
  ...
  const verifyPasswordChange = useAuthStore(
    (state) => state.verifyPasswordChange,
  );
  const isAuthenticated = useAuthStore((state) => state.isAuthenticated);
  const [status, setStatus] = useState("Verifying your password change...");
  ...
  useEffect(() => {
    const verifyToken = async () => {
      try {
        ...
        setStatus("Password change failed. The link may be expired or invalid.");
        console.error(
          "Password Change Verification Error:",
  ...
  }, [token, verifyPasswordChange, navigate, isAuthenticated]);
