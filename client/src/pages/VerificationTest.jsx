...
export default function VerificationTest() {
  ...
  useEffect(() => {
    const fetchTest = async () => {
      try {
        setLoading(true);
        const data = await verificationService.startTest(skillName, "Intermediate");
        setMcqQuestions(data.mcqs);
        setCodeQuestion(data.codeTest);
      } catch (error) {
        toast.error(error.response?.data?.message || "Failed to load test. Session may already exist.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmitTest = async () => {
    ...
    try {
      setSubmitting(true);
      toast.loading("Analyzing code with Gemini and running Tests...", { id: 'grading' });
      ...
      toast.success("Verification Complete! Score: " + result.finalScore + "/100", { id: 'grading' });
      
      setTimeout(() => navigate('/dashboard'), 3000);

    } catch (error) {
      toast.error(error.response?.data?.message || "Submission failed", { id: 'grading' });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0b0b0f] flex items-center justify-center text-indigo-400">
        <span className="animate-pulse">Initializing Verification Engine...</span>
      </div>
    );
  }
        ...
        <div className="w-full bg-white/[0.02] border border-white/10 backdrop-blur-xl rounded-2xl p-6 sm:p-8 shadow-2xl flex flex-col gap-6">
          ...
          <Tabs defaultValue="theory" className="w-full">
            <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 bg-white/5 border border-white/10 p-1 mb-8">
              <TabsTrigger value="theory" className="text-slate-300 data-[state=active]:bg-indigo-500/20 data-[state=active]:text-white transition-all rounded-lg">
                MCQ Theory ({mcqAnswers.length}/5)
              </TabsTrigger>
              <TabsTrigger value="execution" className="text-slate-300 data-[state=active]:bg-indigo-500/20 data-[state=active]:text-white transition-all rounded-lg">
                Compiler Challenge
              </TabsTrigger>
            </TabsList>

            <TabsContent value="theory" className="flex flex-col gap-6 mt-0 focus-visible:outline-none">
              {mcqQuestions.length > 0 && (
                <>
                  <div className="w-full flex items-center gap-4">
                    <Progress value={(mcqAnswers.length / 5) * 100} className="h-2 bg-white/5" />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {mcqQuestions[currentMcqIndex].options.map((option, idx) => {
                      ...
                      const baseClass = "text-left p-5 rounded-xl border transition-all duration-300 ";
                      const activeClass = selected ? "bg-indigo-500/20 border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.2)]" : "bg-white/[0.03] border-white/5 hover:bg-white/10 hover:border-white/20";
                      const textClass = selected ? "text-indigo-100" : "text-slate-300";
                  ...
                  <div className="flex justify-between mt-6 pt-6 border-t border-white/5">
                    <button 
                      onClick={() => setCurrentMcqIndex(curr => Math.max(0, curr - 1))}
                    ...
                    <button 
                      onClick={() => setCurrentMcqIndex(curr => Math.min(4, curr + 1))}
            ...
            <TabsContent value="execution" className="flex flex-col gap-6 mt-0 focus-visible:outline-none">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[600px]">
                  ...
                  <div className="col-span-1 bg-white/5 border border-white/10 rounded-xl p-6 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="mt-8 pt-6 border-t border-white/5">
                      <h3 className="text-xs font-medium text-slate-400 mb-2">Requirements:</h3>
              ...
              <div className="pt-6 mt-6 border-t border-white/5 flex justify-end">
                ...
                >
                  {submitting ? "Analyzing Test Securely..." : "Submit Complete Verification"}
                </button>
              </div>
